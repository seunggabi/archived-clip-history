...
window.$clipHistory.getCookie = (cname) => {
  const name = cname + "=";
...
window.$clipHistory.a = (link, $parents) => {
  const { doms } = window.$clipHistory;
...
window.$clipHistory.r = () => {
  const { a, data, doms } = window.$clipHistory;
